...
Raw unsafe XeSS [`3.0.2`](https://github.com/intel/xess/releases/tag/v3.0.2) bindings for rust. We generate bindings for Vulkan, DirectX12 and DirectX11, for the XeSS-SR (super resolution), XeLL (low latency) and XeSS-FG (frame generation) systems. Further documentation should be found in the XeSS repository at <https://github.com/intel/xess>.
...
Ensure `libxess.dll` is available in the working directory or next to the executable.  Our `xess-bin` crate provides handy paths for copying to the `target/` folder from a `build.rs` script, as long as the crate is pulled from a git or path dependency!
